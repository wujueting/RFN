...
def main():
    ...
    class Subject:
        def __init__(self, Age,Audiogram type,ALB,Degree_of_hearing_loss,MCV,Onset_to_treatment):
            self.age = Age
            self.Audiogram type = Audiogram type
            self.ALB = ALB
        ...
        def make_predict(self):
            subject_data = {
                "Age": [self.Age],
                "Audiogram type": [self.Audiogram type],
                "ALB": [self.ALB],
    ...
    Audiogram type = st.selectbox("Audiogram type (Ascending = 0, flat = 1, descending= 2, profound= 3 )", [0, 1, 2 , 3 ], index=0)
    ...
    if st.button(label="Submit"):
        user = Subject(Age,Audiogram type,ALB,Degree_of_hearing_loss,MCV,Onset_to_treatment)
        user.make_predict()
